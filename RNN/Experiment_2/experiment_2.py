...
param_grid = np.repeat(
    np.array(
        [x for x in itertools.product(n_neurons, epochs, lvar, dim, sigma_rec, weight_decay, threshold, lambda_std)]),
...
for job in job_intervals[task_id - 1]:
 ...
 #   try:
    ...
    # n_active = np.sum(np.std(responses, axis=1) > .1)
    ...
    for run in range(n_runs):
        ...
        random_elements = np.random.choice([-1, 1], size=(responses.shape[0]))
        rot = torch.tensor(np.diag(random_elements)).float()
...
df.to_pickle("Results_2/results_" + str(task_id) + ".pkl")
