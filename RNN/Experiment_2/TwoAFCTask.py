...
def generate_input_target_stream( motion_coh, n_t,
    ...
    # Motion input stream
    motion_input = np.zeros([n_t, 1])
    motion_input[stim_on - 1:stim_off, 0] = motion_coh * np.ones([stim_off - stim_on + 1])
    ...
    target_stream = np.zeros([n_t, 1])
    target_stream[dec_on - 1:dec_off, 0] = np.sign(motion_coh) * np.ones([dec_off - dec_on + 1, 1]).squeeze()
...
def generate_trials( n_trials,n_t = 75):
    ...
    cohs = [-1.    , -0.25  , -0.125 , -0.0625,  0.    ,  0.0625,  0.125 ,
        0.25  ,  1.]
    #cohs = np.linspace(-1,1,25)
    stim_on= int(round(n_t * .2))
